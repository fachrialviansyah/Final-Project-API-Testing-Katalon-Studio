...
def usernameRandom = "fachri." + txtRandom

def responsePOST = WS.sendRequest(findTestObject('Mock API/03 POST User', [('first_name') : 'Fachri', ('last_name') : 'Alvi', ('username') : usernameRandom, ('job_position') : 'Software Quality Assurance', ('job_level') : 'Junior', ('salary') : '7000000', ('work_duration') : '1']))
...
println(val_id)
def responsePOSTduplicate = WS.sendRequest(findTestObject('Mock API/03 POST User', [('first_name') : 'Fachri', ('last_name') : 'Alvi', ('username') : val_username , ('job_position') : 'Software Quality Assurance', ('job_level') : 'Junior', ('salary') : '5000000', ('work_duration') : '1']))
...
//Step Get User By ID
def responseGETbyID = WS.sendRequest(findTestObject('Mock API/02 GET User By ID', [('id') : val_id]))

WS.verifyResponseStatusCode(responseGETbyID, 200)

def idUser = WS.getElementPropertyValue(responseGETbyID, '[0].id')
def firstName = WS.getElementPropertyValue(responseGETbyID, '[0].first_name')
def lastName = WS.getElementPropertyValue(responseGETbyID, '[0].last_name')


WS.verifyElementPropertyValue(responseGETbyID, '[0].id', idUser)
WS.verifyElementPropertyValue(responseGETbyID, '[0].first_name', firstName)
WS.verifyElementPropertyValue(responseGETbyID, '[0].last_name', lastName)


//Step Patch User
def responsePATCH = WS.sendRequest(findTestObject('Mock API/05 PATCH User', [('first_name') : 'Fachri Update', ('last_name') : 'Alvi Update', ('username') : val_username]))
...
def responseGET_userUpdate = WS.sendRequest(findTestObject('Mock API/02 GET User By ID', [('id') : val_idUpdate]))
...
//Step Delete User
def responseDELETE = WS.sendRequest(findTestObject('Mock API/06 DELETE User', [('username') : user_usernameUpdate]))

WS.verifyResponseStatusCode(responseDELETE, 204)

//Step Get User ALL
def responseGETall = WS.sendRequest(findTestObject('Mock API/01 GET Users All'))
